#if !defined NRS_ODE_SYSTEM_DEFS_H
#define NRS_ODE_SYSTEM_DEFS_H
...
namespace NRS::TestBeds
{
	...
	public:
		...
		void derivs(const Real x, const NRS::Vector<Real>& y, NRS::Vector<Real>& dydx) const override
		{
			dydx[0] = y[1];
	...
	static void VanDerPol(Real mju, const Real x, const NRS::Vector<Real>& y, NRS::Vector<Real>& dydx) {
		dydx[0] = y[1];
		dydx[1] = mju * (1.0 - y[0] * y[0]) * y[1] - y[0];
	}
	static void VanDerPolMju0_1(const Real x, const NRS::Vector<Real>& y, NRS::Vector<Real>& dydx) { return VanDerPol(0.1, x, y, dydx); }
